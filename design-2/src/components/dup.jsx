...
const stepImages = [
  images.services.infrastructure,
  images.services.managed,
  images.services.cloud,
  images.services.web,
  images.services.security,
]
...
export default function Framework() {
  ...
  const current = framework[active]
  const color = stepColors[active]

  const go = (dir) => {
    setActive((a) => (a + dir + framework.length) % framework.length)
  }
    ...
    <section id="framework" className="section section-off framework-section">
      <div className="container">
        ...
        <AnimatePresence mode="wait">
          <motion.div
            key={current.letter}
            className="fw-showcase"
            initial={{ opacity: 0, y: 28 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}
          >
            <div className="fw-showcase-visual">
              <img src={stepImages[active]} alt={current.label} />
              <div
                className="fw-showcase-overlay"
                style={{
                  background: `linear-gradient(135deg, ${color}cc 0%, rgba(7,21,40,0.85) 100%)`,
                }}
              />
              <span className="fw-watermark">{current.letter}</span>
              <div className="fw-visual-badge" style={{ background: color }}>
                Step {String(active + 1).padStart(2, '0')}
              </div>
            </div>

            <div className="fw-showcase-body">
              <div className="fw-body-top">
                <div>
                  <span className="fw-pillar" style={{ color }}>{current.letter}</span>
                  <h3>{current.label}</h3>
                </div>
                <div className="fw-nav">
                  <button type="button" onClick={() => go(-1)} aria-label="Previous step">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="M15 18l-6-6 6-6" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </button>
                  <button type="button" onClick={() => go(1)} aria-label="Next step">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <p className="fw-desc">{current.desc}</p>
              <p className="fw-detail">{current.detail}</p>
              ...
              <div className="fw-mini-grid">
                {framework.map((item, i) => (
                  <button
                    key={item.letter}
                    type="button"
                    className={`fw-mini ${i === active ? 'active' : ''}`}
                    onClick={() => setActive(i)}
                    style={{ '--mini-color': stepColors[i] }}
                  >
